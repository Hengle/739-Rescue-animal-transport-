        ...
        for (float z = zStart; z < zEnd; z += resolutionPerMeter)
        {
            for (float x = xStart; x < xEnd; x += resolutionPerMeter)
            {
                for (float y = yStart; y < yEnd; y += resolutionPerMeter)
                {
                    ...
                    {
                        Vector3 pos = new Vector3(x + Random.Range(-halfRes, halfRes), (y + Random.Range(-halfRes, halfRes)) * Random.Range(heightRange.x, heightRange.y), z + Random.Range(-halfRes, halfRes));
