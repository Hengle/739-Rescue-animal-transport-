...
namespace LunarCatsStudio.SuperCombiner {
    ...
    {
        public string versionNumber { get { return "1.6.2"; } }
        ...
        public int _textureAtlasSize = 1024;
		...
		public int _maxVerticesCount = 65534;
        ...
        /// </summary>
        public List<MeshRenderer> _meshList = new List<MeshRenderer>();
        /// <summary>
		/// List of all original SkinnedMeshRenderer in children to combine
        /// </summary>
        public List<SkinnedMeshRenderer> _skinnedMeshList = new List<SkinnedMeshRenderer>();
        /// <summary>
            ...
            for (int i=0; i<_materialsToCombine.Count; i++)
            {
				...
				if (i == _multiMaterialsList.Count && _materialsToCombine[i].Count > 0 || i < _multiMaterialsList.Count && _multiMaterialsList[i].Count > 0)
                {
					...
					foreach (MaterialToCombine mat in _materialsToCombine[i])
                    {
						...
						texturePacker.SetTextures (mat._material, _combineMaterials, materialUVBoundToUse, mat._uvBounds, _tilingFactor);                        
	    ...
	    public void CombineMeshes(List<MeshRenderer> meshesToCombine, List<SkinnedMeshRenderer> skinnedMeshesToCombine, Transform parent)
	    {
	        ...
	        _meshCombiner.SetParameters(_maxVerticesCount, _sessionName, _generateUv2);
        ...
        private List<MeshRendererAndOriginalMaterials> GetMeshRenderersByCombineIndex(List<MeshRenderer> meshRenderers, List<SkinnedMeshRenderer> skinnedMeshRenderers, Transform parent)
        {
            ...
            foreach (MeshRenderer meshRenderer in meshRenderers)
            {
                ...
                for (int i = 0; i < materials.Length; i++)
                {
                    ...
                    {
                        Logger.Instance.AddLog("SuperCombiner", "MeshRenderer of '" + meshRenderer.name + "' has some missing _material references.", Logger.LogLevel.LOG_WARNING);
                    }
                }
                ...
                for (int i = 0; i < _combinedResult._originalMaterialList.Count; i++)
                {
                    if (submeshToCombinedIndex[i].Count > 0)
                    {
                        ...
                        else
                        {
                            ...
                            meshRenderersByCombineIndex[i]._meshRenderers.Add(meshRenderer);
                            meshRenderersByCombineIndex[i]._originalMaterials.Add(meshRenderer.sharedMaterials);
                        }
                    }
                }
            ...
            foreach (SkinnedMeshRenderer skinnedMeshRenderer in skinnedMeshRenderers)
            {
                ...
                for (int i = 0; i < materials.Length; i++)
                {
                    ...
                    {
                        Logger.Instance.AddLog("SuperCombiner", "SkinnedMeshRenderer of '" + skinnedMeshRenderer.name + "' has some missing _material references.", Logger.LogLevel.LOG_WARNING);
                    }
                }
                // If needed, split the submeshes
                bool hasSplitSubmeshes = false;
                for (int i = 0; i < _combinedResult._originalMaterialList.Count; i++)
                {
                    if (submeshToCombinedIndex[i].Count > 0)
                    {
                        ...
                        {
                            // All materials in this meshRenderer correspond to the same combined _index, no need to split submesh
                            meshRenderersByCombineIndex[i]._skinnedMeshRenderers.Add(skinnedMeshRenderer);
                            meshRenderersByCombineIndex[i]._originalskinnedMeshMaterials.Add(skinnedMeshRenderer.sharedMaterials);
                        }
                    }
                }
                // If mesh has been splitted we don't combine mesh, destroy the old meshRenderer and MeshFilter component because there are copies that won't be used anymore
        ...
        private void DisableRenderers(List<MeshRenderer> meshes, List<SkinnedMeshRenderer> skinnedMeshes)
		{
	        foreach (MeshRenderer go in meshes)
	        {
				if (go != null && go.gameObject != _targetGameObject)
	            {
                    ...
                    for (int i = 0; i < lods.Length; i++)
                    {
                        if(i != _managedLodLevel)
                        {
                            ...
                            foreach(Renderer rd in renderers)
                            {
                                ...
                                if (meshrd != null)
                                {
                                    meshRendererList.Remove(meshrd);
                                }
                            }
                        }
                    }
                ...
                foreach (Transform child in children)
                {
                    ...
                    foreach (MeshRenderer mesh in meshes)
                    {
                        ...
                        if (_uniqueCombinedMeshId.ContainsKey(instanceId))
                        {
                            if (meshMaterialId.ContainsKey(_uniqueCombinedMeshId[instanceId]))
                            {
                                ...
                                mesh.GetComponent<MeshFilter>().sharedMesh = meshMaterialId[_uniqueCombinedMeshId[instanceId]];
                            }
                            else
                            {
                                Mesh copyOfMesh = _meshCombiner.copyMesh(mesh.GetComponent<MeshFilter>().sharedMesh);
                            ...
                            for (int j = 0; j < mesh.sharedMaterials.Length; j++)
                            {
                                if (_combineMaterials)
                                {
                                    ...
                                    {
                                        if (mesh.sharedMaterials[j] == _combinedResult._combinedMaterials[k].material)
                                        {
                                            index = k;
                                        }
                                    }
                            ...
                            _toSaveObjectList.Add(mesh);
                        }
                        else
                        {
                            Logger.Instance.AddLog("SuperCombiner", "Could not find " + mesh.name + " in _uniqueCombinedMeshId, data may has been lost, try to combine again before saving.", Logger.LogLevel.LOG_ERROR);
                        }
                    ...
                    foreach (SkinnedMeshRenderer skinnedmesh in skinnedMeshes)
                    {
                        ...
                        if (_uniqueCombinedMeshId.ContainsKey(instanceId))
                        {
                            ...
                            {
                                // This mesh is shared with other game objects, so we reuse the first instance to avoid duplication
                                skinnedmesh.sharedMesh = meshMaterialId[_uniqueCombinedMeshId[instanceId]];
                            }
                            else
                            {
                                Mesh copyOfMesh = _meshCombiner.copyMesh(skinnedmesh.sharedMesh);
                            ...
                            for (int j = 0; j < skinnedmesh.sharedMaterials.Length; j++)
                            {
                                if (_combineMaterials)
                                {
                                    ...
                                    for (int k = 0; k < _combinedResult._combinedMaterials.Count; k++)
                                    {
                                        if (skinnedmesh.sharedMaterials[j] == _combinedResult._combinedMaterials[k].material)
                                        {
                                            index = k;
                                        }
                                    }
                            ...
                            _toSaveSkinnedObjectList.Add(skinnedmesh);
                        }
                        else
                        {
                            Logger.Instance.AddLog("SuperCombiner", "Could not find " + skinnedmesh.name + " in _uniqueCombinedMeshId, data may has been lost, try to combine again before saving.", Logger.LogLevel.LOG_ERROR);
                        }
        ...
        private void createSuperCombinerSettings()
        {
            ...
            // Meshs settings
            _scSettings.meshSettings.maxVerticesCount = _maxVerticesCount;
