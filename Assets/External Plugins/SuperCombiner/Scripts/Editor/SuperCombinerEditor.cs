...
#region Inspector
        ...
        // Constants
        public const int MIN_VERITCES_COUNT = 32;
        public const int MAX_VERITCES_COUNT = 65534;
...
/*#if UNITY_2017_1_OR_NEWER
        ...
        {
            _superCombiner._multipleMaterialsMode = EditorGUILayout.Toggle(new GUIContent("Multiple materials", "The multi _material feature lets you combine to several materials (up to 3) from the listed source materials. This is usually usefull when combining meshes that have various materials (submeshes) that cannot be combined together."), _superCombiner._multipleMaterialsMode);
            if (_superCombiner._multipleMaterialsMode)
            {
                EditorGUILayout.BeginVertical(EditorStyles.helpBox);

                EditorGUILayout.LabelField("Define here every source materials for each combined _material. If more materials than the one listed below are found, they will be automatically assigned to the last combined _material", EditorStyles.wordWrappedLabel);
                ...
                if (_showMultiMaterials)
                {
                    for (int i = 0; i < _superCombiner._multiMaterialsCount; i++)
                    {
                        ...
                        // Source materials
                        EditorGUILayout.PropertyField(_multiMaterialsSC[_multiMaterialsOrder[i]], new GUIContent("source materials (group " + _multiMaterialsOrder[i] + ")", "Define here all the source _material to be included in combined _material " + _multiMaterialsOrder[i]), true);

                        // Remove a _material group button
                        if (GUILayout.Button(new GUIContent("-", "remove this combined _material"), EditorStyles.miniButtonRight, GUILayout.MaxWidth(20f)))
                        {
                            _superCombiner._multiMaterialsCount--;
                            _serializedCombiner.Update();
        ...
        private void DisplayMeshesSettingsSection ()
        {
			...
			if (_showMeshSettings)
            {
				...
				if (_superCombiner._combineMeshes) {
					...
					_superCombiner._maxVerticesCount = EditorGUILayout.IntSlider (new GUIContent ("Max vertices count", "If the combined mesh has more vertices than this parameter, it will be split into various meshes with 'Max vertices count' vertices"), _superCombiner._maxVerticesCount, MIN_VERITCES_COUNT, MAX_VERITCES_COUNT, GUILayout.ExpandWidth (true));
				}
